...
var requestAsync = require('urllib-sync').request;
...
Client.prototype.requestSync = function(opts) {
  var client = this;
  ...
  var req = requestAsync(url, {
    method: opts.method,
    headers: opts.headers,
    data: opts.body
    });
  return parser.parse(req.data);
};
...
Client.prototype.querySync = function(query_id) {
  return this.requestSync({ method: 'GET', path: '/v1/query/' + query_id });
};
...
Client.prototype.execute = function(opts, callback) {
  if (callback)
    return this.executeResource(opts, callback, false);
  else
    this.statementResource(opts);
};

Client.prototype.executeSync = function(opts) {
    return this.executeResource(opts, null, true);
};

Client.prototype.executeResource = function(opts, callback, sync) {
  var client = this;
